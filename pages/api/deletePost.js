...
async function uploadHandler(req, res) {
    ...
    try {
        if (req.method === 'POST') {
            ...
            const getPostData = await db.db('personal-blog').collection('blog-post').findOne({ _id: ObjectId(postId) })
            if (getPostData) {
                await db.db('personal-blog').collection('blog-post').deleteOne({ _id: ObjectId(postId) })
                await db.db('personal-blog').collection('deleted-post').insertOne({ ...getPostData, metadata: { id: postId }, timestamp: new Date() })
